...
const NAV_ITEMS = [
  { label: 'Dashboard', icon: LayoutDashboard, href: '/restaurant/dashboard', key: 'dashboard' },
  { label: 'Menu', icon: Menu, href: '/restaurant/menu', key: 'menu' },
  { label: 'Orders', icon: ShoppingCart, href: '/restaurant/orders', key: 'orders' },
  { label: 'Analytics', icon: BarChart3, href: '/restaurant/analytics', key: 'analytics' },
];

const MORE_OPTIONS = [
  { label: 'Help & Support', icon: HelpCircle, href: '#', key: 'help' },
  { label: 'Feedback', icon: MessageSquare, href: '#', key: 'feedback' },
  { label: 'Settings', icon: Settings, href: '/restaurant/settings', key: 'settings' },
];
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[color:var(--color-bg-primary)] border-t border-[color:var(--color-border-light)] flex items-center justify-around z-[var(--z-fixed)] px-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={hideLabels ? 20 : 24} className='shrink-0'/>
              {!hideLabels && (
        ...
        <button
          ...
          title="More options"
        >
          <MoreHorizontal size={hideLabels ? 20 : 24} className='shrink-0'/>
          {!hideLabels && <span className="text-xs font-medium">More</span> }          
        </button>
      </nav>
